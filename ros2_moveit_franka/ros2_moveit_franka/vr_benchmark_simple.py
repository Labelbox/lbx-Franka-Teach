...
class VRBenchmarkNode(Node):
    ...
    def generate_vr_perturbations_at_75hz(self, duration: float) -> List[VRPose]:
        ...
        for i in range(num_samples):
            ...
            # Position perturbations - MODERATE and SMOOTH for safe movement
            x = 0.05 * np.sin(2 * np.pi * 0.5 * t)      # 5cm @ 0.5Hz - smooth and safe
            y = 0.03 * np.sin(2 * np.pi * 0.3 * t)      # 3cm @ 0.3Hz - gentle  
            z = 0.04 * np.sin(2 * np.pi * 0.7 * t)      # 4cm @ 0.7Hz - moderate
            
            # Rotation perturbations - MUCH SMALLER and SMOOTHER
            roll = 0.1 * np.sin(2 * np.pi * 0.4 * t)    # ~6° @ 0.4Hz - gentle
            pitch = 0.08 * np.sin(2 * np.pi * 0.6 * t)  # ~4.6° @ 0.6Hz - smooth
            yaw = 0.06 * np.sin(2 * np.pi * 0.2 * t)    # ~3.4° @ 0.2Hz - safe
        ...
        smoothed_perturbations = self.smooth_vr_perturbations(perturbations)
        self.get_logger().info(f'Applied smoothing filter to reduce motion violations')
        
        return smoothed_perturbations
    
    def smooth_vr_perturbations(self, perturbations: List[VRPose], window_size: int = 5) -> List[VRPose]:
        """Apply moving average smoothing to VR perturbations to reduce abrupt changes"""
        if len(perturbations) < window_size:
        ...
        for i in range(len(perturbations)):
            ...
            end_idx = min(len(perturbations), i + half_window + 1)
            
            # Collect positions and orientations in window
            positions = []
            orientations = []
            triggers = []
            
            for j in range(start_idx, end_idx):
                positions.append(perturbations[j].position)
                orientations.append(perturbations[j].orientation)
                triggers.append(perturbations[j].trigger_value)
            
            # Calculate smoothed values using moving average
            smoothed_pos = np.mean(positions, axis=0)
            smoothed_quat = np.mean(orientations, axis=0)
            smoothed_trigger = np.mean(triggers)
            
            # Normalize quaternion after averaging
            quat_norm = np.linalg.norm(smoothed_quat)
            if quat_norm > 0:
                smoothed_quat = smoothed_quat / quat_norm
            
            # Create smoothed VR pose
            smoothed_pose = VRPose(
                position=smoothed_pos,
                orientation=smoothed_quat,
                timestamp=perturbations[i].timestamp,
                trigger_value=smoothed_trigger
            )
    ...
    def run_benchmark(self):
        ...
        # Test large movement first to verify robot can move visibly
        self.get_logger().info('\n' + '='*60)
        self.get_logger().info('🧪 TESTING LARGE MOVEMENT FIRST')
        self.get_logger().info('='*60)
        
        if self.test_large_movement():
            self.get_logger().info('✅ Large movement test passed - robot moves visibly!')
            self.get_logger().info('   Now proceeding with VR benchmark...')
            time.sleep(2.0)
        else:
            self.get_logger().error('❌ Large movement test failed - robot may not be moving!')
            self.get_logger().error('   Check RViz or robot status. Continuing anyway...')
            time.sleep(2.0)
        
        # Run VR tests
        self.get_logger().info('\n' + '='*60)
        self.get_logger().info('🎮 STARTING VR PERTURBATION TESTS')
        self.get_logger().info('='*60)
        ...
        try:
            ...
            if ik_response and ik_response.error_code.val == 1:
                ...
                if len(joint_positions) == 7:
                    # Create trajectory with smooth velocity/acceleration limits
                    ...
                    # Add conservative velocity and acceleration limits for smooth motion
                    # These are much safer than the default limits
                    point.velocities = [0.1] * 7   # 0.1 rad/s max velocity (~5.7°/s) - very conservative
                    point.accelerations = [0.1] * 7  # 0.1 rad/s² max acceleration - gentle
                    
                    # Set timing for smooth motion (longer duration for high-frequency commands)
                    movement_duration = max(0.1, duration * 2.0)  # At least 100ms, usually longer
                    point.time_from_start.sec = int(movement_duration)
                    point.time_from_start.nanosec = int((movement_duration - int(movement_duration)) * 1e9)
    ...
    def test_large_movement(self):
        """Test a single large movement to verify robot moves visibly"""
        self.get_logger().info('🧪 Testing large robot movement for visibility...')
        ...
        # Create a large, obvious movement - move joint 1 by 30 degrees
        target_joints = current_joints.copy()
        target_joints[0] += 0.52  # +30 degrees on joint 1
        
        self.get_logger().info(f'🎯 Target joints:  {[f"{j:.3f}" for j in target_joints]}')
        self.get_logger().info('📐 Joint 1 movement: +30° - SHOULD BE VERY VISIBLE')
        
        # Create trajectory with conservative limits
        trajectory = JointTrajectory()
        trajectory.joint_names = self.joint_names
        ...
        point.velocities = [0.2] * 7      # 0.2 rad/s max velocity (~11.5°/s) - conservative
        point.accelerations = [0.1] * 7   # 0.1 rad/s² max acceleration - gentle
        
        # Longer duration for smooth movement
        point.time_from_start.sec = 5  # 5 second movement - very slow and safe
        ...
        if not self.trajectory_client.server_is_ready():
            self.get_logger().error('Trajectory server not ready')
            return False
        
        self.get_logger().info('🚀 Sending large movement command...')
        future = self.trajectory_client.send_goal_async(goal)
        
        # Wait for goal acceptance and execution (longer for smooth movement)
        import time
        time.sleep(6.0)  # Wait for 5-second movement to complete + buffer
        
        # Check final position
        final_joints = self.get_current_joint_positions()
        if final_joints:
            self.get_logger().info(f'📍 Final joints:   {[f"{j:.3f}" for j in final_joints]}')
            movement = abs(final_joints[0] - current_joints[0])
            movement_deg = movement * 57.3
            self.get_logger().info(f'📐 Actual movement: {movement_deg:.1f}° on joint 1')
            
            if movement > 0.1:  # More than ~6 degrees
                self.get_logger().info('✅ LARGE MOVEMENT SUCCESSFUL - Robot should be visibly moved!')
                return True
            else:
                self.get_logger().warn('⚠️  Movement too small - robot may not be moving')
                return False
