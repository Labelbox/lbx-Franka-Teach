...
class VRBenchmarkFakeNode(Node):
    def __init__(self):
        ...
        # Test frequencies - very conservative for testing
        self.target_rates_hz = [1, 2, 5]  # Very slow for fake hardware testing
    ...
    def create_test_poses(self) -> List[Pose]:
        ...
        # Up pose - clearly different
        up_pose = Pose()
        up_pose.position.x = 0.4
        up_pose.position.y = 0.0
        up_pose.position.z = 0.6    # 20cm higher
        up_pose.orientation.w = 1.0
        poses.append(up_pose)
        
        # Down pose - clearly different
        down_pose = Pose()
        down_pose.position.x = 0.4
        down_pose.position.y = 0.0
        down_pose.position.z = 0.2   # 20cm lower
        down_pose.orientation.w = 1.0
        poses.append(down_pose)
