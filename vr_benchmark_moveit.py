...
from vr_benchmark.msg import VRPose
...
class VRBenchmarkMoveItNode(Node):
    ...
    def go_to_home_pose(self) -> bool:
        ...
        if success:
            ...
            time.sleep(1.0)  # Brief pause at home
            return True
        else:
    ...
    def generate_vr_perturbations(self, duration: float, hz: float) -> List[VRPose]:
        ...
        for i in range(num_samples):
            ...
            # LARGE perturbations for clearly visible movement (like fake hardware test)
            x = 0.15 * np.sin(2 * np.pi * 0.2 * t)      # 15cm @ 0.2Hz - very visible
            y = 0.12 * np.sin(2 * np.pi * 0.15 * t)     # 12cm @ 0.15Hz - visible  
            z = 0.10 * np.sin(2 * np.pi * 0.3 * t)      # 10cm @ 0.3Hz - noticeable
            
            # Large rotation perturbations for visible movement
            roll = 0.2 * np.sin(2 * np.pi * 0.1 * t)    # ~11° @ 0.1Hz - clearly visible
            pitch = 0.15 * np.sin(2 * np.pi * 0.12 * t) # ~8.6° @ 0.12Hz - visible
            yaw = 0.1 * np.sin(2 * np.pi * 0.08 * t)    # ~5.7° @ 0.08Hz - noticeable
        ...
        self.get_logger().info(f'Generated {len(perturbations)} LARGE perturbations at {hz}Hz for visibility')
    ...
    def benchmark_control_rate(self, target_hz: float) -> BenchmarkResult:
        ...
        self.get_logger().info(f'      Movement range: ±15cm position, ±11° rotation - SHOULD BE VISIBLE!')
    ...
    def print_summary(self):
        ...
        if self.benchmark_results:
            ...
            print(f"   Movement range: ±15cm position, ±11° rotation")
            print(f"   Home resets: Before/after each test for consistency")
            print(f"   Check RViz: Robot should move visibly around home position")
